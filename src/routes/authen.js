"use strict";
const helper = require("../helpers/hhelper");
...
authen_router.get("/lock_screen", (req, res) => {
...
const display_startup_from = (err, params) => {
	...
	params.res.render('login', params_out);
};
...
const send_error_or_log_uer = (err, params) => {
	if ( err !== null ) {
		...
		if ( params.is_match === false ) {
			params.errors = ["Username or Password do not match."];
		} else {
			log_user_session ( null, { req: params.req, user_info: {"_id": 2, "username": "Darren", "user_email": "Darren"}});
			params.res.redirect("/");
		}
	}
